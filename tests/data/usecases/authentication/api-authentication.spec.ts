...
describe('ApiAuthentication', () => {
  ...
  test('Should throw UnexpectedError if HttpPostClient returns stauts 400', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
  ...
  test('Should throw UnexpectedError if HttpPostClient returns stauts 500', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
  ...
  test('Should throw UnexpectedError if HttpPostClient returns stauts 404', async () => {
    const { sut, httpPostClientSpy } = makeSut()
    httpPostClientSpy.response = {
  ...
  test('Should return an AccountModel if HttpPostClient returns stauts 200', async () => {
    const { sut, httpPostClientSpy } = makeSut()
